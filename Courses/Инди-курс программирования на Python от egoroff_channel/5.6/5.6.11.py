...
использует исходное бинарное черно-белое изображение и полученный Мишиной программой негатив,
...
orig_list = [input() for _ in range(n)]
empty = input()
misha_list = [input() for __ in range(n)]
count = 0
B = 'B'
W = 'W'
neg_list = []
...
for a in range(n):
    ...
    for b in range(m):
        if orig_list[a][b] == 'B':
            ...
            str_list += 'B'
    neg_list.append(str_list)

for i in range(n):
    for j in range(m):
        if neg_list[i][j] != misha_list[i][j]:
            count += 1
